...
class InstructionDecoder:
    ...
    3. 移除 '0 == NOP' 的判斷，0x00000000 為有效的 R_TWB 指令。
    4. [本次修正] B_SYNC 新增 [13:8] Reason Code 解析 (NOP 類型細分)。
    """
    def __init__(self):
        # Opcode 定義
        self.OP_MAP = {
        ...
        # [新增] B_SYNC 原因代碼對照表 (Payload [13:8])
        self.SYNC_REASON_MAP = {
            0x00: "Std NOP",          # 標準手動 NOP
            0x01: "WB Drain Protect", # WB Drain 保護
            0x02: "AGU Busy Wait",    # AGU Busy 等待
            0x03: "Blind Spot Gap",   # Tile 訊號盲區
            0x04: "R_CW Delay",       # R_CW Safety Delay
            0x05: "Smart Sync Stall", # Smart Sync Stall
            0x06: "Global Barrier",   # Global Barrier
            0x07: "External Wait",    # External Wait
            0x08: "Scale Mode Gap"    # Scale Mode Gap
        }
    ...
    def decode_32bit(self, val):
        ...
        if op_name == "R_TWB":
            ...
            # [修正] 解析 Reason Code [13:8]
            reason_code = (payload >> 8) & 0x3F
            reason_str = self.SYNC_REASON_MAP.get(reason_code, f"Reason({reason_code})")
            ...
            # [邏輯調整] 
            # 只有當 Reason Code 為 0 (Std NOP) 且沒有任何等待位元時，才視為 "視覺上的 NOP (Hidden)"
            # 如果 Reason Code > 0 (例如 WB Drain)，即使沒有等待位元，也視為有效指令並顯示出來。
            is_waiting = (wait_tile or wait_agu1 or wait_agu0 or ext_wait or sync_mask > 0)
            
            if reason_code == 0 and not is_waiting:
                return None, "NOP" # 隱藏標準空指令
            ...
            if sync_mask > 0:
                ...
                waits.append(f"Slots:[{','.join(mask_list)}]")

            wait_str = ", ".join(waits) if waits else "None"
            
            # 組合顯示資訊
            details = (f"<b>Type:</b> {reason_str}<br>"
                       f"<b>Wait:</b> {wait_str}")
            ...
            details = "<b>Core Output Data</b><br>Trigger Output Buffer"
...
def visualize_interactive_detailed(timeline, output_file="schedule_detailed.html"):
    ...
    fig.update_layout(
        title=f"DTCore Detailed Schedule: {output_file} ({num_cycles} Cycles)",
        xaxis=dict(
            title="Time (Cycles)",
            tickmode="linear",
            dtick=1,
...
if __name__ == "__main__":
    ...
    # 定義要處理的任務清單
    tasks = [
        #("output/gemm_conflict_free/instruction.mem", "schedule_conflict_free.html"),
        ("output_verification/instruction.mem", "schedule.html")
    ]
    
    for mem_file, output_html in tasks:
        print(f"\n--- Processing: {mem_file} ---")
        data = decoder.parse_mem_file(mem_file)
        
        if data:
            visualize_interactive_detailed(data, output_file=output_html)
        else:
            print(f"Skipping {mem_file} because it could not be read or is empty.")